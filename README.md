...
- **Pricing grid** — Gemini, OpenAI, Anthropic, Mistral with **Context / tier** column. Where tiered pricing exists (e.g. ≤200K vs >200K), **all tiers** are shown (one row per tier). Data from `pricing.json` + `src/data/pricingTiersOverlay.js`. **Retired** badge marks deprecated models.
