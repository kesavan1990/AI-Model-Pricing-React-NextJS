...
- **Data status (footer)** — Footer shows **Pricing: [date]; Benchmarks: [date]** so you can see when each dataset was last updated; if the benchmark pipeline fails, the benchmarks date may be older or "—". See [Data status (footer)](docs/UI.md#data-status-footer).
