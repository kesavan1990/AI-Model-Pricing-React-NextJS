...
- **OpenAI:** Tiered pricing (Batch, Standard, Priority) and extended-context rates (e.g. over 272K tokens); single rate shown; link to [official pricing](https://platform.openai.com/docs/pricing). Retired: e.g. `gpt-4-0314`, `gpt-4-1106-preview`, `gpt-3.5-turbo-instruct`, `babbage-002`, `davinci-002`; `isRetiredOpenAIModel()`.
- **Anthropic:** Service tiers (Batch, Standard, Priority) and extended context (over 200K tokens); single rate shown; link to [official pricing](https://docs.anthropic.com/en/docs/about-claude/pricing). Retired: e.g. Claude 3 Opus, 3.5 Haiku, 3.7 Sonnet, `claude-3-haiku`; `isRetiredAnthropicModel()`.
- **Mistral:** Legacy models deprecated in favor of newer versions; link to [official pricing](https://mistral.ai/pricing). Retired: e.g. `mistral-large`, `mistral-small`, `mistral-medium-2312`, `open-mistral-nemo`; `isRetiredMistralModel()`.

Retired detection is in `src/render.js`; the badge appears in the provider’s pricing table and in the **Model comparison** table.
