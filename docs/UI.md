...
**Provider + retired filtering** — Models from pricing data are shown unless the **name clearly belongs to another provider** (`getProviderByModelName` vs bucket) or the model is **retired/deprecated** (see [ALLOWED_MODELS.md](ALLOWED_MODELS.md), [RETIRED_MODELS.md](RETIRED_MODELS.md)). Unknown new IDs are **kept** in the bucket supplied by the API/file. (1) **State:** `filterToAllowedModels()` then `filterRetiredModels()` run inside `processPayload()` (PricingContext / `lib/dataPipeline.js`). (2) **Lists:** `getAllModels()` and `getUnifiedCalcModels()` require `isAllowedModel` and `!isRetired`. **Sections affected:** Overview, **Models**, **Value Analysis**, **Calculators**, **Benchmarks**, **Recommend**. Excluded examples: Gemini 1.0/1.5, `gemini-pro`; OpenAI deprecated list (e.g. `gpt-4-0314`, `babbage-002`); Anthropic Claude 3.x; Mistral legacy names (e.g. `mistral-large`, `mistral-small` without version).
...
**Provider match + retired (excluded in all sections)** — **`isAllowedModel`** in **`src/data/allowedModels.js`** removes cross-provider mis-tags; **retired** models are excluded via **`src/utils/retiredModels.js`**. **App state:** **PricingContext** calls `processPayload()` (`lib/dataPipeline.js`), which runs `filterToAllowedModels()` then `filterRetiredModels()`. **Model lists:** `src/calculator.js` uses `isAllowedModel()` and `isRetired()` in `getAllModels()` and `getUnifiedCalcModels()`. See [ALLOWED_MODELS.md](ALLOWED_MODELS.md) and [RETIRED_MODELS.md](RETIRED_MODELS.md).
...
**Plain-language UX** — A **Quick guide** box above the table explains that 0–100 columns differ from big **ranking** numbers, what the colored dots mean, and that **—** is “not on that list,” not zero. Column headers use friendly labels (**Knowledge**, **Coding** + subtitle, **Reasoning**, **Chat rank**, **Code rank**, **Doc rank**, **Cost** + $–$$$). The expandable **Where do these numbers come from?** table uses columns **What you see**, **In plain words**, and **Where we get it**. Radar spokes use short labels (**Knowledge**, **Coding (est.)**, **Chat rank**, **Reasoning**).

**Alignment** — Header text is **left-aligned**; export buttons **right-aligned** in the same row. The sources table is **full width** with aligned columns (horizontal scroll on small screens). Numeric score columns use **right-aligned** headers and cells, **tabular numerals**, and sort controls **right-aligned** in headers. **Model** names are **left-aligned** with `word-break` for long IDs.

**Heatmap** — **Stronger / Middle / Lower** bands (0–100-style columns); ranking columns map ELO into the same colors after scaling. **—** when unmatched on a leaderboard.
...
**Benchmark pipeline** — The UI loads both `pricing.json` and `benchmarks.json` and merges by **model name** and **provider**. The benchmark script fetches LMArena **text**, **code**, and **document** leaderboards (**ELO** each; see [BENCHMARKS.md](BENCHMARKS.md)) and **Hugging Face Open LLM Leaderboard** (MMLU, reasoning), merges with pricing models, and writes `benchmarks.json`. When external data is missing, embedded scores from `getBenchmarkForModel()` in `src/calculator.js` are used. See [Benchmark pipeline](BENCHMARKS.md).
...
The **📜 History** button in the header opens a **Pricing history** modal. The modal merges **two sources**: (1) **`pricing-history.json`** on the same host (daily snapshots from **GitHub Actions**, IST-oriented labels), and (2) **`localStorage`** snapshots from this browser. After each successful **initial** pricing load and after **Refresh**, **`syncHistoryAfterPricingLoad()`** (`src/pricingService.js`) merges the server file into **`localStorage`** via **`syncMergedHistoryToLocalStorage()`** so server-only days appear without opening the modal first; **`getServerHistory()`** retries several times on failure. When you **open** the modal, it runs **`syncMergedHistoryToLocalStorage()`** again and then reads **`getHistory()`**, so the merge is **persisted** to **`localStorage`** before the list is shown. A **local** daily snapshot is still added on first visit of an IST calendar day **only if** that day is not already present after the merge (see **`getHistoryDateMergeKey`** in **`src/historyDateKey.js`**). In the modal you can **compare two dates**: select two snapshots and see which models had price changes (drops and increases) between those dates. **Export CSV** and **Export PDF** download the merged list shown in the modal. After **Refresh from web**, a **Recent price changes** summary may appear in the footer (legacy **`index.html`** path where implemented). **Pipeline:** [PRICING_UPDATES.md](PRICING_UPDATES.md) § Pricing history; **React:** `components/HistoryModal.js`, **`context/PricingContext.js`**.
