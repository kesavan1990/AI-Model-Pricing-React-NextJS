...
**Frontier tooltips** — Three places explain what “frontier” means. (1) **Section subtitle:** A **(?)** icon next to “frontier (best value at each cost level)” with a `title` attribute. (2) **Legend hint above the chart:** A dedicated line “Colored points = **Frontier (best value)** (?)” appears directly above the chart; the **(?)** uses a **custom CSS hover tooltip** (no delay): on hover, a small card-style box appears above it with the full explanation (“Frontier = models with best performance at their cost; no cheaper model scores higher on the selected metric. Hover a colored point for details.”). This uses `data-tooltip` and `::after` in CSS so the tooltip is always visible on hover. Markup: `<p class="value-chart-legend-hint">` and `<span class="value-chart-frontier-tooltip" data-tooltip="...">(?)</span>` in `index.html`; styles `.value-chart-legend-hint`, `.value-chart-frontier-tooltip`, and `.value-chart-frontier-tooltip:hover::after` in `css/styles.css`. (3) **Chart point tooltip:** When you hover a frontier (colored) point, the Chart.js tooltip shows model name, provider, cost per request, performance score, and “✓ Frontier — best value at this cost (no cheaper model has higher performance).” Implementation: tooltip callback in `renderQuadrantChart()` in `src/valueChart.js` adds the frontier explanation when `onFrontier` is true.
...
**Implementation** — `src/valueChart.js`: `mergeModels()` builds cost + performance per model from `getAllModels(data)` and `getBenchmarkForModelMerged()`; `computeCostPerRequest()` uses (prompt/1e6)×input + (output/1e6)×output; `computeFrontier()` implements the frontier algorithm; `renderQuadrantChart()` uses **Chart.js** (scatter only: all models + frontier points). The chart is rendered or updated when the Compare tab is active and when data or filters change; theme (dark/light) is respected. Markup: `#section-value-chart`, `#value-chart-canvas`, `.value-chart-controls`, `.value-chart-legend-hint`, `.value-chart-frontier-tooltip` in `index.html`; styles in `css/styles.css` (`.value-chart-section`, `.value-chart-wrap` with fixed height, `.value-chart-legend-hint`, `.value-chart-frontier-tooltip` and its hover `::after` for the custom Frontier tooltip).
